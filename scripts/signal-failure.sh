...
case "$1" in
	...
	HANAINSTALLFAIL) 
	    log "The HANA installation did not succeed. Please check installation media."
	    curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "The HANA installation did not succeed. Please check installation media.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    echo curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "The HANA installation did not succeed. Please check installation media.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    ;;
	    
  HANACHECKFAIL)
      log "The HANA version check did not suceed. Please check installation media."
      curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "The HANA version check did not suceed. Please check installation media.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    echo curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "The HANA version check did not suceed. Please check installation media.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    ;;
	
	TMPFSFAIL)
      log "Unable to mount SAP HANA TMPFS for Fast Restart."
      curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "Unable to mount SAP HANA TMPFS for Fast Restart.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    echo curl -X PUT -H 'Content-Type:' --data-binary '{"Status" : "FAILURE","Reason" : "Unable to mount SAP HANA TMPFS for Fast Restart.","UniqueId" : "HANAMaster","Data" : "Failure"}' "${WaitForMasterInstallWaitHandle}"
	    ;;
	INCOMPATIBLE)
	    log "Instance Type = X1 and O.S. is not supported with X1"
  ...
  INCOMPATIBLE_RHEL) 
    log "Instance Type = X1 and RHEL 6.6 is not supported with X1"
  ...
  INCOMPATIBLE_RHEL_SCALEOUT)
    log "RHEL 6.7 is not supported with X1 Scaleout"
  ...
  YUM)
    log "Instance Type = X1 and yum repo is not supported"
  ...
  ZYPPER)
    log "Instance Type = X1 and zypper repo is not supported"
  ...
  ACTMODULEFAIL)
    log "`date` SUSE public cloud module activation FAILED"
  ...
  EMPTY_STORAGE_JSON) 
    log "`date` Valid storage.json file not found"
